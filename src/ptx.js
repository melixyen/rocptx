...
function openWithProxy(fm, method, url, headerObj) {
    ...
    var useProxy = proxy.isActiveFor(url);
    fm.open(method, useProxy ? proxy.resolveURL(url) : url);
    if (!useProxy) return merged;
    var extra = proxy.headers || {};
    for (var k in extra) {
...
var proxy = {
    ...
    enable: function (url, options) {
        var nextURL = normalizeProxyURL(url, 'enable');
        if (options && options.headers !== undefined) {
            if (!options.headers || typeof (options.headers) != 'object' || Array.isArray(options.headers)) throw 'rocptx.proxy.enable: options.headers must be an object';
            proxy.headers = Object.assign({}, options.headers);
        }
        proxy.url = nextURL;
        proxy.enabled = true;
        return proxy;
    ...
    //更換 proxy 網址，不改變啟用狀態
    setURL: function (url) {
        proxy.url = normalizeProxyURL(url, 'setURL');
        return proxy;
    },
    //回傳實際要送出的網址：啟用時把 TDX 官方網域換成 proxy 網址，否則原樣回傳
    resolveURL: function (url) {
        if (!proxy.isActiveFor(url)) return url;
        return String(proxy.url).replace(/\/+$/, '') + url.slice(TDX_ORIGIN.length);
    },
    //此網址送出時是否會走 proxy
    isActiveFor: function (url) {
        return !!(proxy.enabled && proxy.url && isTDXURL(url));
    }
};
